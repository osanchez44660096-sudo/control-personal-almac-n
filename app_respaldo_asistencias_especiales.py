...
def dashboard():
    ...
    activos = Trabajador.query.filter_by(
        estado="ACTIVO"
    ).count()

    cesados = Trabajador.query.filter_by(
        estado="CESADO"
    ).count()
    ...
    <p><b>Trabajadores registrados:</b> {total}</p>
    <p><b>Activos:</b> {activos}</p>
    <p><b>Cesados:</b> {cesados}</p>
    ...
    <ul>
        ...
        <li>
        <a href="/reporte_horas">
        REPORTE HORAS EXTRAS
        </a>
        </li>

        <li>
<a href="/reporte_movimientos">
...
def reporte_horas():
    ...
    registros = HorasExtras.query.order_by(
        HorasExtras.id.desc()
    ).all()
